...
const SignedIn: FC<Props> = ({ session: {user} }) => {
  ...
  return (
    <div className="relative" style={{ zIndex: 100000000000000 }}>
      <button
        className={`${
          showOptions ? "bg-dark-blue/60" : "bg-form-blue"
        } border border-light-blue/30 p-1 px-2 flex items-center justify-around rounded-md`}
      ...
      {showOptions && (
        <div className="absolute overflow-hidden left-0 top-full w-full rounded bg-light-blue mt-1 text-black">
          <Link href={`/user/${user?.name}/product`}>
            <button
